...
export default function BookingModal({ 
  ...
  // Fix: Ensure dates are never empty
  ...
  const handleConfirmBooking = async () => {
    if (!customerName || !customerEmail || !customerPhone) {
      setSubmitError("Please fill all contact details.");
      return;
    }
    ...
    try {
      ...
      // Validate UUID format for room_id only
      const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
      const validRoomId = uuidRegex.test(room?.id) ? room.id : '1cff9f52-513d-4a30-89dc-b2d6fa357842';

      const { error: dbError } = await supabase.from('bookings').insert([{
        room_id: validRoomId,
        hotel_id: "GREEN_GARDEN_RESORT", // Now works because SQL Step 1 changed column to TEXT
      ...
      await emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, {
        ...
        room_type: room?.room_type || "Luxury Room",
        ...
        <div className="hidden md:flex w-1/3 bg-[#0f172a] p-10 flex-col justify-between text-white text-left">
          <div>
            ...
            <p className="mt-4 text-slate-400 text-sm leading-relaxed">
              Experience the perfect blend of comfort and luxury at Green Garden.
            </p>
          </div>
          <div className="bg-white/5 border border-white/10 p-5 rounded-2xl flex items-start gap-3">
            <Info size={18} className="text-emerald-400" />
            <p className="text-[11px] text-slate-300">Request handled by 24/7 reception.</p>
          </div>
        </div>
        ...
        <div className="flex-1 p-8 lg:p-12 bg-white overflow-y-auto max-h-[90vh]">
          {submitSuccess ? (
            <div className="text-center py-16">
              ...
              <button onClick={onClose} className="mt-10 px-12 py-4 bg-slate-900 text-white rounded-2xl font-bold">Done</button>
            </div>
          ) : (
            <div className="space-y-8 text-left">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="md:col-span-2">
                  ...
                  <input className="w-full px-5 py-4 rounded-2xl border border-slate-200 bg-slate-50 outline-none focus:border-emerald-500 transition-all" 
                    value={customerName} onChange={e => setCustomerName(e.target.value)} placeholder="Full Name" />
                </div>
                <div>
                  <label className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2"><Mail size={12}/> Email</label>
                  <input className="w-full px-5 py-4 rounded-2xl border border-slate-200 bg-slate-50 outline-none focus:border-emerald-500 transition-all" 
                    value={customerEmail} onChange={e => setCustomerEmail(e.target.value)} placeholder="email@address.com" />
                </div>
                <div>
                  <label className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2"><Phone size={12}/> Phone</label>
                  <input className="w-full px-5 py-4 rounded-2xl border border-slate-200 bg-slate-50 outline-none focus:border-emerald-500 transition-all" 
                ...
                <div>
                  ...
                  <input type="date" className="w-full px-5 py-4 rounded-2xl border border-slate-200 bg-slate-50 outline-none" 
              ...
              <div className="p-8 bg-slate-50 rounded-[2.5rem] border border-slate-100 mt-10 flex flex-col sm:flex-row justify-between items-center gap-8">
                ...
                <button onClick={handleConfirmBooking} disabled={isSubmitting} className="w-full sm:w-auto px-12 py-6 bg-[#10b981] text-white rounded-3xl font-black text-xl shadow-2xl shadow-emerald-200 hover:bg-emerald-600 active:scale-95 disabled:bg-slate-300 transition-all">
